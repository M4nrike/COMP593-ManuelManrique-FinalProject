...
def init_apod_cache():
    ...
    if os.path.isdir(image_cache_dir):
        print(f"image_cache_directory:{image_cache_dir} \n Image cache directory already exists.")       
    else:           
...
def add_apod_to_cache(apod_date):
  ...
  #  print(info_dict)
    ...
    v_ext = (v_file_p [-4:])
    vtitaux = v_title2 + v_ext
    # sha256 = apod_api.image_sha256
    sha256 = "Valor sha256"
   # print("Valores extraidos del APOD image:")
  #  print(f"titulo:{v_title2}\n explicacion:{v_expla}\n tipo:{v_type}\n ruta:{v_file_p}")
    add_apod_to_db(vtitaux, v_expla, v_type, v_file_p, sha256)
    return 0 
...
def add_apod_to_db(title, explanation, tipo, file_path, sha256):
    ...
    cur.execute('SELECT * FROM ImagesCache')
